<template>
  ...
  >
    ...
    <div class="sidebar-header" :class="{ 'sidebar-header-collapsed': sidebarCollapsed }">
      ...
      <div class="sidebar-logo flex h-9 w-9 items-center justify-center overflow-hidden rounded-md border border-[var(--ui-border)] bg-[var(--ui-surface)]">
        <img v-if="settingsLoaded" :src="siteLogo || '/logo.png'" alt="Logo" class="h-full w-full object-contain" />
      </div>
      <div class="sidebar-brand" :class="{ 'sidebar-brand-collapsed': sidebarCollapsed }" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">
        <span class="sidebar-brand-title text-sm font-semibold text-[var(--ui-text)]">
          {{ siteName }}
        </span>
        <!-- Version Badge -->
      ...
      <template v-if="isAdmin">
        ...
        <div v-if="!authStore.isSimpleMode" class="sidebar-section">
          ...
          <template v-for="item in personalNavItems" :key="item.path">
            <button
              v-if="item.rechargeAware"
              type="button"
              class="sidebar-link mb-1 w-full"
              :class="{ 'sidebar-link-active': isActive(item.path), 'sidebar-link-collapsed': sidebarCollapsed }"
              :title="sidebarCollapsed ? item.label : undefined"
              @click="handleRechargeNav(item.path)"
            >
              <span v-if="item.iconSvg" class="h-5 w-5 flex-shrink-0 sidebar-svg-icon" v-html="sanitizeSvg(item.iconSvg)"></span>
              <component v-else :is="item.icon" class="h-5 w-5 flex-shrink-0" />
              <span class="sidebar-label" :class="{ 'sidebar-label-collapsed': sidebarCollapsed }" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">{{ item.label }}</span>
...
interface NavItem {
  ...
  /** 充值入口项:点击走 useRecharge(外链优先,否则内置 /purchase),不走普通路由跳转。 */
  rechargeAware?: boolean
  children?: NavItem[]
  /**
...
//
// 条目顺序：密钥 → 用量 → 费率说明 → 可用渠道 → 渠道状态 → 订阅/支付 → 兑换/资料。
// 费率说明紧跟使用记录，让用户能从历史扣费自然跳到当前费率口径。
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/billing-rates', label: t('nav.billingRates'), icon: PriceTagIcon, hideInSimpleMode: true },
    ...
    { path: '/purchase', label: t('nav.buySubscription'), icon: RechargeSubscriptionIcon, hideInSimpleMode: true, featureFlag: () => rechargeVisible.value, rechargeAware: true },
...
// 充值入口项点击:先做移动端关闭等通用处理,再走 useRecharge(外链优先/内置兜底)。
function handleRechargeNav(path: string) {
