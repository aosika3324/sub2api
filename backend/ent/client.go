...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		...
		c.EditableFileArtifact, c.EditableFileTask, c.ErrorPassthroughRule, c.Group,
		...
		c.UserPlatformQuota, c.UserSubscription, c.VideoGeneration,
	} {
		n.Use(hooks...)
	}
}
...
func (c *ImageGenerationClient) Hooks() []Hook {
	hooks := c.hooks.ImageGeneration
	return append(hooks[:len(hooks):len(hooks)], imagegeneration.Hooks[:]...)
}

// Interceptors returns the client interceptors.
func (c *ImageGenerationClient) Interceptors() []Interceptor {
	inters := c.inters.ImageGeneration
	return append(inters[:len(inters):len(inters)], imagegeneration.Interceptors[:]...)
}
...
type (
	hooks struct {
		...
		ChannelMonitorRequestTemplate, EditableFileArtifact, EditableFileTask,
		ErrorPassthroughRule, Group, IdempotencyRecord, IdentityAdoptionDecision,
		ImageConversation, ImageGeneration, PaymentAuditLog, PaymentOrder,
		PaymentProviderInstance, PendingAuthSession, PromoCode, PromoCodeUsage, Proxy,
		RedeemCode, SecuritySecret, Setting, SubscriptionPlan, TLSFingerprintProfile,
		UsageCleanupTask, UsageLog, User, UserAllowedGroup, UserAttributeDefinition,
		UserAttributeValue, UserPlatformQuota, UserSubscription,
		VideoGeneration []ent.Hook
	}
	inters struct {
		APIKey, Account, AccountGroup, Announcement, AnnouncementRead, AuthIdentity,
		AuthIdentityChannel, BatchImageEvent, BatchImageItem, BatchImageJob,
		...
		UsageCleanupTask, UsageLog, User, UserAllowedGroup, UserAttributeDefinition,
		UserAttributeValue, UserPlatformQuota, UserSubscription,
		VideoGeneration []ent.Interceptor
	}
)
