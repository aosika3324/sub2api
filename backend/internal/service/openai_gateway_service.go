...
// the single source of truth shared with the exported ComputeImageCostBreakdown
// helper so studio cost figures match deductions.
...
func (s *OpenAIGatewayService) ComputeImageCostBreakdown(ctx context.Context, user *User, apiKey *APIKey, result *OpenAIForwardResult) *CostBreakdown {
	...
	// as RecordUsage does. ApplyOpenAIImageBillingResolution is idempotent, so
	// RecordUsage's later call is a harmless no-op.
