...
func IsImageGenerationIntent(endpoint string, requestedModel string, body []byte) bool {
	...
	// Fork: Images-API style requests (prompt/n/images on /responses) are image intent.
...
func IsExplicitImageGenerationIntent(endpoint string, requestedModel string, body []byte) bool {
	...
	// Fork: Images-API style requests (prompt/n/images on /responses) are an
	// explicit image request, so they must gate capability routing too.
	if isOpenAIResponsesEndpoint(endpoint) && openAIResponsesImagesAPIFieldsIndicateImage(body) {
		return true
	}
	if len(body) == 0 || !gjson.ValidBytes(body) {
