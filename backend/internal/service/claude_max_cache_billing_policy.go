...
func hasTopLevelEphemeralCacheControl(parsed *ParsedRequest) bool {
	body := parsedRequestBodyBytes(parsed)
	if len(body) == 0 {
		return false
	}
...
func parsedRequestBodyBytes(parsed *ParsedRequest) []byte {
...
func parsedSystemValue(parsed *ParsedRequest) any {
	if parsed == nil {
		return nil
	}
	system, ok := parsed.SystemValue()
	if !ok {
		return nil
	}
	return system
...
func parsedMessagesValue(parsed *ParsedRequest) []any {
	if parsed == nil {
		return nil
	}
	var messages []any
...
func analyzeClaudeCacheProjection(parsed *ParsedRequest) claudeCacheProjection {
	...
	switch system := parsedSystemValue(parsed).(type) {
	case string:
	...
	for _, rawMsg := range parsedMessagesValue(parsed) {
...
func countExplicitCacheBreakpoints(parsed *ParsedRequest) int {
	...
	total := 0
	if system, ok := parsedSystemValue(parsed).([]any); ok {
		for _, raw := range system {
...
func estimateLastUserMessageTokens(parsed *ParsedRequest) int {
	messages := parsedMessagesValue(parsed)
	if len(messages) == 0 {
		return 0
	}
